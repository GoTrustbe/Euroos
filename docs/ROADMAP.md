...
- **Remaining (large, follow-on):** the **busybox / curl / sqlite** breadth pass against a real `libc.so`, which surfaces many more syscalls plus symbol versioning, TLS relocations, and `IRELATIVE`/ifunc resolution.
...
- **Remaining (follow-on):** f32/f64 ops; the full WASI preview1 surface (real `fd_read`/`path_open`/`sock_*` wired to EuroFS/EuroNet); a `wasm <file>` shell command to run `.wasm` from the VFS.

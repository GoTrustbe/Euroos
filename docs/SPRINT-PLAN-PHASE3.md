...
| 3F-5 | EuroSuite ↔ OS integration | ES-Int | 🟢 **MIME + default-app done + boot-verified; rest remains** — new crate [`euromime`](../crates/euromime): **MIME-type detection** by **magic bytes** (spoof-resistant — a PNG mislabelled `.txt` is still `image/png`) then extension, with ZIP refined to the Office/ODF type, and a **default-app `Registry`** (type → app) with user overrides. `[3f5]` resolves real files on the live FS (a real `.docx`→Writer, image→viewer, text→editor, override honoured); `open <path>` / `open --set` shell commands; 7 host tests. **REMAINING:** wire double-click in the file manager to launch the resolved app window; CoW version-history UI (= S snapshots); document sandboxing (macros never auto-run). |
| 3F-6 | **Audio routing & desktop integration** **[NEW]** | 🟢 **core done + boot-verified** | New [`euroaudio::Router`](../crates/euroaudio/src/router.rs): the audio-server layer over the mixer — **per-app streams** (each with its own volume + mute), **per-app routing** to output **devices**, a **default-device policy** with **hotplug** (unplugging the default re-routes its streams + picks a new default), and a **master** volume/mute; `render(device)` mixes exactly the streams on that device (via the host-tested `mix`) into the PCM the HDA driver plays. `[3f6]` + 6 host tests (euroaudio 9→15); `audio` shell command shows the device list + per-app mixer. **REMAINING:** the live per-period pull from the HDA driver (hardware-gated, 3B-7) + a graphical mixer applet. |

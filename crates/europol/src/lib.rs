//! EuroPol — een **declaratieve policy-engine** (plan X).
//!
//! EuroGuard-capabilities zijn een technisch-correcte enforcement-laag, maar
//! beheerders denken niet in bits — ze denken in beleid: *"deze applicatie mag geen
//! internet."* EuroPol vertaalt een leesbare policy (TOML-achtig) naar een
//! **EuroGuard-capability-masker** + pad-/netwerk-regels, met `[allow]`/`[deny]`-
//! secties waar **deny altijd wint**. De kernel dwingt het resultaat af in het
//! syscall-pad (de bestaande capability-check). Pure `no_std`-logica → host-getest.
...
// ── EuroGuard-capability-bits (gespiegeld uit `kernel::ring3`) ───────────────
...
/// Map een capability-naam naar z'n bit.
...
/// De naam van een (enkele) capability-bit — voor `explain`/listings.
...
impl Policy {
    /// Het effectieve capability-masker: voeg `allow` toe aan `base`, trek dan `deny`
    /// af. **Deny wint altijd** (ook over een verleende basis-capability).
    pub fn effective_caps(&self, base: u64) -> u64 {
        (base | self.allow_caps) & !self.deny_caps
    }

    /// Mag deze capability? (deny-bit wint; anders allow-bit; anders standaard-deny).
    ...
    /// Mag dit pad? Een `deny`-prefix wint; anders een `allow`-prefix; anders deny.
    ...
    /// Leg uit WAAROM een capability is toegestaan/geweigerd (voor `europol explain`).
    pub fn explain_cap(&self, cap: u64) -> String {
        let n = cap_name(cap);
        if self.deny_caps & cap != 0 {
            alloc::format!("{n}: GEWEIGERD door [deny].capabilities in policy '{}'", self.name)
        } else if self.allow_caps & cap != 0 {
            alloc::format!("{n}: toegestaan door [allow].capabilities in policy '{}'", self.name)
        } else {
            alloc::format!("{n}: GEWEIGERD (niet in [allow] van policy '{}' — standaard-deny)", self.name)
        }
    }
}

/// Parse een policy-tekst (TOML-achtig): `name = "..."`, secties `[allow]`/`[deny]`,
/// en keys `capabilities = ["CAP_X", ...]` / `paths = ["/a", "/b"]` /
/// `log_denied = true`.
...
/// Parse een TOML-array `["a", "b", "c"]` → de losse elementen (zonder quotes).
...
mod tests {
    ...
    fn parse_and_effective_caps() {
        ...
        // Effectief: een proces met ALLE rechten verliest CAP_IMMUTABLE_ADMIN.
        let all = CAP_CONSOLE | CAP_PROC_INFO | CAP_FILE | CAP_NET | CAP_IMMUTABLE_ADMIN;
        let eff = p.effective_caps(all);
        assert_eq!(eff & CAP_IMMUTABLE_ADMIN, 0); // deny wint
        assert_ne!(eff & CAP_NET, 0);
    }

    #[test]
    fn deny_wins_over_allow() {
        // Zelfde cap in allow én deny → geweigerd.
    ...
    fn cap_checks() {
        ...
        assert_eq!(p.check_cap(CAP_PROC_INFO), Decision::Deny); // niet in allow → standaard-deny
    }
    ...
    fn path_checks() {
        ...
        assert_eq!(p.check_path("/var/random"), Decision::Deny); // niet in allow
    }

    #[test]
    fn explain_is_human_readable() {
        let p = parse(FIREFOX);
        assert!(p.explain_cap(CAP_NET).contains("toegestaan"));
        assert!(p.explain_cap(CAP_IMMUTABLE_ADMIN).contains("GEWEIGERD door [deny]"));
        assert!(p.explain_cap(CAP_PROC_INFO).contains("standaard-deny"));
    }
}
